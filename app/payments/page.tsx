...
export default function PaymentsPage() {
  ...
  const getAuthHeaders = useCallback(() => {
    if (!accessToken) {
      toast({
        title: "Xatolik",
        description:
          "Avtorizatsiya tokeni topilmadi. Iltimos, qayta tizimga kiring.",
        variant: "destructive",
      });
      ...
      try {
        ...
        if (error.message !== "Unauthorized") {
          ...
          toast({
            title: "Xatolik",
            description:
              error.message || `${entityName} yuklashda noma'lum xatolik.`,
            variant: "destructive",
          });
        }
        setData([]);
    ...
    async (
      ...
      try {
        ...
        if (currentFilters.paymentType !== "all")
          queryParams.append("payment_type", currentFilters.paymentType);
        if (currentFilters.apartment !== "all")
          queryParams.append("apartment", currentFilters.apartment);
        if (currentFilters.dueDate !== "all")
          queryParams.append("due_date", currentFilters.dueDate);
        if (currentFilters.durationMonths !== "all")
          queryParams.append("duration_months", currentFilters.durationMonths);
        ...
        if (!response.ok) {
          ...
          const errorData = await response
            .json()
            .catch(() => ({ detail: "Server javobi xato." }));
          throw new Error(
            `To'lovlarni olishda xatolik (${response.status}): ${
      ...
      for (let i = 0; i < updatedApartments.length; i++) {
        try {
          ...
          const balanceResponse = await fetch(
            `${API_BASE_URL}/apartments/${updatedApartments[i].id}/get_total_payments/`,
            {
              headers,
            }
          );
          if (balanceResponse.ok) {
            ...
            if (data && typeof data === "object") {
              ...
              updatedApartments[i].total_amount = String(
                data.total_amount || 0
              );
            } else {
              updatedApartments[i].balance = "0";
              updatedApartments[i].total_amount =
                updatedApartments[i].price || "0";
            }
          } else {
            updatedApartments[i].balance = "0";
            updatedApartments[i].total_amount =
          ...
          const overdueResponse = await fetch(
            `${API_BASE_URL}/apartments/${updatedApartments[i].id}/overdue_payments/`,
            {
              headers,
            }
          );
          if (overdueResponse.ok) {
            ...
            updatedApartments[i].overdue_payments =
              overdueData?.overdue_payments || [];
            updatedApartments[i].total_overdue =
          ...
          console.error(
            `Xonadon ${updatedApartments[i].id} ma'lumotlarini olishda xato:`,
            error
          );
          updatedApartments[i].balance = "0";
    ...
    async (token: string) => {
      ...
      try {
        const results = await Promise.allSettled([
          fetchApiData<Apartment>(
            `${API_BASE_URL}/apartments/?page_size=1000`,
            token,
            (data) => {
          ...
          fetchApiData<Client>(
            `${API_BASE_URL}/users/?user_type=mijoz&page_size=1000`,
            token,
            setClients,
            "Mijozlar"
          ),
          fetchApiData<ObjectData>(
            `${API_BASE_URL}/objects/?page_size=1000`,
            token,
            setObjects,
            "Obyektlar"
          ),
  ...
  const filteredPayments = useMemo(() => {
    ...
    if (filters.object !== "all") {
      const objectId = parseInt(filters.object, 10);
      const apartmentIdsInObject = apartments
  ...
  const statistics = useMemo(() => {
    ...
    const filteredApartmentIds = new Set(
      filteredPayments.map((p) => p.apartment)
    );
    apartments
      .filter((apt) => filteredApartmentIds.has(apt.id))
      .forEach((apartment) => {
        if (
          apartment.overdue_payments &&
          Array.isArray(apartment.overdue_payments)
        ) {
          apartment.overdue_payments.forEach((overdue) => {
            if (overdue && typeof overdue === "object" && "amount" in overdue) {
              total_overdue +=
                parseFloat(overdue.amount?.toString() || "0") || 0;
            }
          });
        }
      });
  ...
  const handleFilterChange = (value: string, field: keyof typeof filters) => {
    setFilters((prev) => ({
      ...
      ...(field === "paymentType" &&
        value !== "muddatli" && { dueDate: "all", durationMonths: "all" }),
    }));
  };
  ...
  const handleAddPayment = async () => {
    ...
    if (
      ...
      (formData.payment_type === "muddatli" &&
        (!formData.duration_months || !formData.due_date)) ||
      ...
      toast({
        title: "Xatolik",
        description:
          "Majburiy maydonlarni to'ldiring va to'g'ri qiymatlar kiriting.",
        variant: "destructive",
      });
      setActionLoading(false);
      return;
    ...
    const monthlyPayment =
      durationMonths > 0 ? remainingAmount / durationMonths : 0;
    ...
    const paymentData = {
      ...
      duration_months:
        formData.payment_type === "muddatli" ? durationMonths : null,
      monthly_payment:
        formData.payment_type === "muddatli" ? monthlyPayment.toFixed(2) : null,
      due_date:
        formData.payment_type === "muddatli"
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ detail: "Server javobini o'qishda xato." }));
        throw new Error(
      ...
      if (apartment) {
        const newStatus =
          formData.payment_type === "band" ? "band" : "sotilgan";
      ...
      if (accessToken) {
        ...
        const updatedApartments = await fetchApartmentDetails(
          accessToken,
          apartments
        );
        setApartments(updatedApartments);
  ...
  const handleDeletePayment = async (paymentId: number) => {
    ...
    if (
      !window.confirm(
        `ID ${paymentId} to'lov yozuvini o'chirishni tasdiqlaysizmi? Bu amalni orqaga qaytarib bo'lmaydi.`
      )
    )
      return;
    ...
    try {
      ...
      if (!response.ok && response.status !== 204) {
        const errorData = await response
          ...
          .catch(() => ({ detail: "Server javobini o'qishda xato." }));
        throw new Error(
          `To'lovni o'chirishda xatolik (${response.status}): ${
            errorData.detail || response.statusText
          }`
        );
      }
  ...
  const filteredApartmentsForSelect = useMemo(() => {
    ...
    return apartments
      .filter(
        (apt) => apt.object != null && apt.object.toString() === filters.object
      )
    ...
    return (
      <div className="flex min-h-screen flex-col">
        ...
        <footer className="border-t py-4 px-4 text-center text-sm text-muted-foreground mt-auto">
          Version 1.0 | Barcha huquqlar ximoyalangan | Ushbu Dastur CDCGroup
          tomonidan yaratilgan | CraDev Company tomonidan qo'llab quvvatlanadi |
          since 2019
        </footer>
      </div>
    );
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Filtr bo'yicha jami to'langan summa
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-blue-600">
                {formatCurrency(statistics.total_remaining)}
              </div>
              <p className="text-xs text-muted-foreground">
                Filtr bo'yicha jami qoldiq summa
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Filtr bo'yicha muddati o'tgan summa
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex flex-wrap gap-4 items-center p-4 border rounded-md bg-card">
          ...
          <Select
            ...
            disabled={
              paymentsLoading ||
              actionLoading ||
              (filteredApartmentsForSelect.length === 0 &&
                filters.object !== "all")
            }
          >
            ...
            <SelectContent>
              ...
              {filteredApartmentsForSelect.length === 0 &&
                filters.object !== "all" && (
                  <p className="p-2 text-sm text-muted-foreground">
                    Tanlangan obyektda xonadon yo'q
                  </p>
                )}
              {apartments.length === 0 && filters.object === "all" && (
                <p className="p-2 text-sm text-muted-foreground">
                  Xonadonlar yuklanmagan
                </p>
              )}
            </SelectContent>
          </Select>
                    ...
                    filteredPayments.map((payment, index) => {
                      ...
                      return (
                        <TableRow
                          ...
                          className={cn(
                            "hover:bg-muted/50",
                            deletingPaymentId === payment.id && "opacity-50"
                          )}
                        >
                          ...
                          <TableCell>
                            {payment.user_fio || `ID: ${payment.user}`}
                          </TableCell>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[480px]">
          <DialogHeader>
            ...
            <DialogDescription>
              (*) majburiy maydonlar. Muddatli to'lov uchun to'liq ma'lumot
              kiriting.
            </DialogDescription>
          </DialogHeader>
